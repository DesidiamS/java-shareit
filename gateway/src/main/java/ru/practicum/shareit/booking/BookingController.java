...
public class BookingController {
    ...
    @GetMapping("/owner")
    public ResponseEntity<Object> getBookingByOwner(@RequestParam @Nullable BookingState state,
                                                    @RequestHeader(USER_HEADER) long userId) {
        return bookingClient.getBookingByOwner(userId, state);
    }

    @PatchMapping("/{bookingId}")
    public ResponseEntity<Object> changeBookingStatus(@RequestHeader(USER_HEADER) long userId,
                                                      @PathVariable Long bookingId,
                                                      @RequestParam Boolean approved) {
        return bookingClient.bookingChangeStatus(userId, bookingId, approved);
    }
